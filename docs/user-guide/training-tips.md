...
6. 开启 **Block 交换**（`blocks_to_swap`）：把靠后的若干 DiT 层放在内存、算到才搬进
   显存。Anima 每层约省 0.13GB，换出全部 28 层后训练可在 6GB 级显卡上跑（1024²、
   sample 照开，速度约慢 11%）。换出的层需要等量的**可锁定**内存，内存余量要够。
...
### 启动时报 CUDA out of memory，但显存明明够用

**症状**：开了 Block 交换后，模型一层都还没上卡就报 `CUDA error: out of memory`。

**原因**：这不是显存，是**主机页锁定内存**（`cudaHostAlloc`）。换出层的权重要锁在系统
内存里，而 Windows 对可锁定内存有系统级上限（经验上约为物理内存的一半）。

**处理**：调小 `blocks_to_swap`，或关掉其他占内存的程序。现在报错文案会直接说明是页锁定
内存、本次要锁多少，不再是笼统的一句 CUDA out of memory。
