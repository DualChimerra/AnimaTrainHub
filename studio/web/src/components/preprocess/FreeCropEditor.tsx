...
export default function FreeCropEditor({
  ...
  useEffect(() => {
    ...
    // Pointer events, not mouse events: a finger on a phone/tablet never
    // produces mousemove, so a mouse-only drag could not move or resize a rect.
            ...
            >
              ...
              {isSel && HANDLES.map((h) => (
                <div
                  ...
                  onPointerDown={(e) => onPointerDown(e, 'resize', c, h)}
                />
              ))}
            </div>
          )
