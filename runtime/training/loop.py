...
def _save_adapter(ctx, path) -> None:
    """落盘 adapter 权重；EMA 启用且影子已建立时**额外**存一份 ``*_ema.safetensors``。

    正常文件照存不误——EMA 是补充选项而非替代，用户要能两个都试。EMA 版通过
    ``ema.applied()`` 临时换入影子权重后复用同一个 ``injector.save``，所以 alpha
    重写 / ss_* metadata / 族标记全都一致。
    """
...
def run(ctx: TrainingContext) -> None:
    ...
    # DOP 差分输出保持：启动期把能不能跑先确定下来（适配器要能临时关自己、
    # 触发词必须有），别让训练跑到一半才发现配置无效。
    ...
    if _dop_enabled:
        ...
        if not _dop_trigger:
            raise ValueError(
                "dop_enabled=true 需要非空的 trigger_word："
                "DOP 的保持分支就是「把触发词从 caption 里去掉」的那一条，"
                "没有触发词两条分支完全一样、约束退化成 0。"
                " / DOP needs a trigger word: set it on the Train page"
                " (Trigger word card) — it must appear in the captions."
            )
        logger.info(
            "[DOP] 差分输出保持已启用：trigger=%r，weight=%.3g，ratio=%.2f"
            "（每个启用的 step 多两次前向）",
            _dop_trigger,
    ...
    for epoch in range(ctx.start_epoch, args.epochs):
        ...
        for batch_idx, batch in enumerate(ctx.dataloader):
            ...
            with torch.autocast("cuda", dtype=ctx.dtype):
                ...
                # DOP 差分输出保持（标准路径；leap / navit 在 schema 层互斥）。
                # 同一批图、去掉触发词的 caption，比较「有适配器」与「关掉适配器」的
                # 预测：教会 LoRA「没有触发词时什么都不改」，顺带堵死「抄数据集内容」
                # 这条路——两条分支的内容完全一样，抄它拿不到任何奖励。详见 training/dop.py
                if (
                    _dop_enabled
            ...
            if is_group_end:
                ...
                # 权重 EMA：只在真实 update step 之后累计（不是每个 micro-batch），
                # 否则 grad_accum 会让「有效窗口」随累计步数漂移
