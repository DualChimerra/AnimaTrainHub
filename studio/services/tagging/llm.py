...
def _openai_compatible_endpoint(base_url: str, *, kind: str) -> str:
    ...
    if not base:
        raise RuntimeError("The LLM base_url is empty")
...
def _iter_sse_payloads(raw: str) -> Iterator[dict[str, Any]]:
    ...
    def flush() -> Iterator[dict[str, Any]]:
        ...
        try:
            ...
            raise RuntimeError(f"The LLM SSE response is not valid JSON: {data[:200]}") from exc
...
def _extract_sse_response_text(raw: str, *, endpoint: str) -> str:
    ...
    for payload in _iter_sse_payloads(raw):
        ...
        if final:
            final_text = final
    if not seen_payload:
        raise RuntimeError("The LLM SSE response was empty")
...
def fetch_openai_compatible_models(
    ...
    *,
    ...
    if resp.status_code >= 400:
        raise RuntimeError(f"Could not read the model list (HTTP {resp.status_code}): {resp.text[:300]}")
    payload = resp.json()
...
def test_openai_compatible_connection(
    ...
    *,
    ...
    try:
        ...
        if not result["ok"]:
            result["error"] = "The LLM returned empty content"
        return result
...
class LLMTagger:
    ...
    def prepare(self) -> None:
        ...
        if not ok:
            raise RuntimeError(f"The LLM tagger is unavailable: {msg}")
    ...
    def _call_with_retry(
        ...
        *,
        ...
        raise RuntimeError(f"The LLM call failed after {cfg.max_retries} retries: {last_exc}")
    ...
    def _parse_json_text(content: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"The LLM response is not valid JSON: {text[:200]}") from exc
        if not isinstance(parsed, dict):
            raise RuntimeError("The top level of the LLM JSON must be an object")
        return parsed
        ...
        *,
        ...
        with Image.open(path) as img:
            ...
            while True:
                ...
                if side <= 256:
                    raise RuntimeError(
                        f"The image is still over the {max_image_mb:g} MB limit after compression — raise max_image_mb or use a smaller image"
                    )
