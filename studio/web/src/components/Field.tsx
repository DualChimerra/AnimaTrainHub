...
const FieldHint = ({ children }: { children: React.ReactNode }) => (
  <span className="ml-2 text-2xs font-medium text-warn align-middle">{children}</span>
)
...
export default function Field({
  ...
  const helpNode = (help || recommend) ? (
    <div className="mt-1">
      {help && <div className="text-xs text-fg-tertiary">{help}</div>}
      {recommend && (
        <div className="text-xs text-accent/80 mt-0.5">
          <span className="opacity-70">{t('field.recommended')}: </span>{recommend}
        </div>
      )}
    </div>
  ) : null
...
function TextareaField({
  label, helpNode, value, onChange, disabled = false, hintNode,
}: TextareaFieldProps) {
...
function NumberField({
  label, kind, helpNode, value, defaultValue, minimum, maximum,
...
function PathStringField({
  name, label, kind, helpNode, value, onChange, disabled = false, hintNode, suffix,
}: PathFieldProps) {
  const { t } = useTranslation()
