...
class Block(nn.Module):
    ...
    def forward(
        ...
        *,
        packed_tokens: bool = False,
        **packed_kwargs,
    ) -> torch.Tensor:
        # ``packed_tokens=True`` dispatches to ``forward_tokens`` (NaViT/FiT packed
        # path). The packed loop MUST route through ``__call__`` rather than calling
        # ``forward_tokens`` directly: nn.Module forward/backward hooks only fire on
        # ``__call__``, and block swap hangs its weight prefetch/release on exactly
        # those four hooks (``training/block_swap.py::attach``). A direct method call
        # silently skips them — every swapped block then computes with whatever pair of
        # weights the two GPU slots happen to hold, on the right device, so there is no
        # error and no NaN, just wrong weights and (per ``attach``'s own docstring)
        # silently wrong gradients.
        #
        # ``packed_kwargs`` is forwarded verbatim to ``forward_tokens``; on the dense
        # path it must be empty (guarded below) so a misspelled kwarg still raises.
        if packed_tokens:
        ...
        if packed_kwargs:
            raise TypeError(
                f"Block.forward() got unexpected keyword arguments "
                f"{sorted(packed_kwargs)} (packed_tokens=False)"
            )
    ...
    def forward_tokens(
        ...
        if token_wise_mod and mod_index is not None:
            _sel = lambda t: t.index_select(1, mod_index)   # [1,G,D] → contiguous [1,ΣN,D]
        elif token_wise_mod:
            _sel = lambda t: t
        else:
            _sel = lambda t: t[:, :1, :]
...
class MiniTrainDIT(nn.Module):
    ...
    def forward_packed_navit(
        ...
        for block in self.blocks:
            # Routed through ``__call__`` (``packed_tokens=True`` → ``forward_tokens``),
            # never ``blk.forward_tokens(...)`` directly: block swap's prefetch/release
            # hooks fire on ``__call__`` only. Mirrors the dense checkpointed loop in
            # ``training/families/anima/forward.py``, so both paths share the exact same
            # hook semantics (incl. the recompute behaviour under gradient checkpointing).
        ...
        # Direct call is fine here: block swap only manages ``self.blocks`` (see
        # ``place_model_for_block_swap``'s ``blocks.{i}.`` prefixes), so final_layer is
        # always GPU-resident and carries no swap hooks.
