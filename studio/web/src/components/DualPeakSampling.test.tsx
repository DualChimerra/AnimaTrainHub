...
describe('Dual Peak sampling controls', () => {
  ...
  it('shows custom controls only for dual_peak, preserves values and edits the second peak', async () => {
    ...
    expect(screen.queryByText('Timestep Shift')).not.toBeInTheDocument()
    ...
    expect(screen.queryByText('Dual Peak Peak1 Position')).not.toBeInTheDocument()
    expect(screen.getByText('Timestep Shift')).toBeInTheDocument()
