...
def _validate_run_id(run_id: str) -> str:
    if not isinstance(run_id, str) or not _RUN_ID_RE.fullmatch(run_id):
        raise EvalSamplesError(f"Invalid eval sample run id: {run_id!r}")
    return run_id
...
def _rel_to_version(version_dir: Path, path: Path) -> str:
    try:
        ...
        raise EvalSamplesError(f"The path does not belong to this version: {path}") from exc
...
def _resolve_checkpoint(version_dir: Path, raw_path: str | None) -> dict[str, Any]:
    ...
    if raw_path:
        ...
        if not raw:
            raise EvalSamplesError("checkpoint_path must not be empty")
        candidate = Path(raw)
        ...
        try:
            ...
            raise EvalSamplesError("checkpoint_path must be inside the version's output/ directory") from exc
        if path.suffix.lower() != ".safetensors" or not path.is_file():
            raise EvalSamplesError(f"The checkpoint does not exist or is not a .safetensors file: {raw_path}")
        ...
        if not ckpts:
            raise EvalSamplesError("There are no LoRA checkpoints under the version's output/ directory")
        item = ckpts[0]
...
def load_run(version_dir: Path, run_id: str, eval_root: Path | None = None) -> dict[str, Any] | None:
    ...
    try:
        ...
        raise EvalSamplesError(f"Could not read the eval sample run: {exc}") from exc
    if not isinstance(data, dict):
        raise EvalSamplesError("The eval sample run must be a JSON object")
    return data
...
def create_run(
    ...
    *,
    ...
    if not items:
        raise EvalSamplesError("Nothing to evaluate: validation/ is empty and no sample prompts are configured")
...
def run_sample_job(
    ...
    *,
    ...
    if run is None:
        raise EvalSamplesError(f"Eval sample run not found: {run_id}")
    if int(run.get("project_id") or 0) != int(project["id"]):
        raise EvalSamplesError("This eval sample run belongs to a different project")
    if int(run.get("version_id") or 0) != int(version["id"]):
        raise EvalSamplesError("This eval sample run belongs to a different version")
...
def sample_image_path(version_dir: Path, run_id: str, filename: str, eval_root: Path | None = None) -> Path:
    if "/" in filename or "\\" in filename or filename in {"", ".", ".."}:
        raise EvalSamplesError(f"Invalid sample filename: {filename!r}")
    if not filename.lower().endswith(_IMAGE_EXT):
        raise EvalSamplesError("Eval sample images must be .png")
    ...
    try:
        ...
        raise EvalSamplesError(f"The sample image path escapes its directory: {filename!r}") from exc
    return resolved
